...
/**
 * YSM 模型 extra 动作表索引（D1/D2）：对 T6 调用方暴露的稳定 API。
...
public final class YsmExtraAnimationIndex {
    private static volatile Map<String, ModelAnimations> index = Map.of();
    private static volatile boolean scanned = false;

    private YsmExtraAnimationIndex() {
    }

    /** 未命中返回 {@link ModelAnimations#EMPTY}（root 与 submenus 都空） */
    public static ModelAnimations lookup(String modelId) {
        ensureScanned();
        return index.getOrDefault(modelId, ModelAnimations.EMPTY);
    }
    ...
    public static synchronized void refresh() {
        ...
        index = new LinkedHashMap<>(YsmModelScanner.scan(roots, locale));
        scanned = true;
        TlmStatueAnimation.LOGGER.info("YSM extra animation index refreshed: {} models indexed", index.size());
    }
